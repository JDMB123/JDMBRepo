...
* Make a new class. This time, let's increase the number of buckets as
  the size of the set increases. The goal is to have `store.length >
  N` at all times.
...
* Use `Integer#hash` as a base hashing function. The trick here will be to create a
  scheme to convert instances of each class to a `Integer` and then apply this
...
Once you are finished with phases 1 - 5 head over to Part Two. Accessing this on GitHub? Use [this link][github-lru-part-two].

[github-lru-part-two]: https://github.com/appacademy/curriculum/blob/master/ruby/projects/hash_map_lru_cache/part_two.md
