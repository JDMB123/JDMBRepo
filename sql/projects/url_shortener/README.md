...
the `users` table. Accessing this on GitHub? Use [this link][github-indexing-reading]. This will allow us to look up rows in the table
...
Add uniqueness and presence [validations][validations-reading]. Accessing this on GitHub? Use [this link][github-validations-reading]. Without
these, people might go about creating user accounts without emails. We
cannot tolerate such misbehavior.

[indexing-reading]: indices-overview
[github-indexing-reading]: https://github.com/appacademy/curriculum/blob/master/sql/readings/indexing.md
[validations-reading]: basic-validations
[github-validations-reading]: https://github.com/appacademy/curriculum/blob/master/sql/readings/validations.md
...
First we'll need to create a `visits` joins table in our database. Make a new migration to create the `visits` table and determine the columns needed for the table. Add appropriate indices and validations.
Be sure to include timestamps. We'll need them later.
