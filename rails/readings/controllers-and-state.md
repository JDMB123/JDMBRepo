...
* Facebook presents a login page when you go to facebook.com and are
   not logged in.
* User fills out the web form, presses "submit". The web browser
   makes an HTTP request to POST credentials to the Facebook's server
   (perhaps to `/session`).
* Facebook server verifies username/password. Sends a redirect to the
   browser instructing it to GET `/feed`.
* Client makes GET request for `/feed`.
